...
- `vertical_video` 直式影片檔：確認 9:16 直式影片可播放，長度符合 Shorts/Reels/TikTok 節奏。
...
- `guardian_universe_match` 守護者宇宙一致：使用 艾莉絲 / iris 的色彩、象徵物與情緒設定，不混用其他守護者。
  - evidence：畫面截圖或素材清單。
- `caption_quiz_cta` Caption 單一 CTA：Caption 主 CTA 維持完成 15 題測驗，不加入商品、Luna 或聯盟導購作為第一動作。
...
post_url: https://www.youtube.com/shorts/replace-with-real-post-url
views: 0
site_clicks: 0
quiz_starts: 0
quiz_completions: 0
