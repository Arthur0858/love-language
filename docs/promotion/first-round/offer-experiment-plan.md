...
- `identity_save`: 測驗完成 >= 10，`free_keepsake_downloads` + `keepsake_clicks` >= 2。
- `owned_lead`: 測驗完成 >= 10，`supply_lead_requests` + `contact_requests` >= 2。
- `luna_soft_offer`: 測驗完成 >= 10，`luna_pack_clicks` + `luna_clicks` >= 1。
- `affiliate_book`: 測驗完成 >= 10，`affiliate_book_clicks` + `resources_clicks` >= 2。
...
- 狀態：HOLD
- 假設：免費收藏物能提高守護者認同、保存與回訪。
- 目前數據：quiz=0，free_keepsake_downloads=0，keepsake_clicks=0
- 資產：`iris-free_story_card_upgrade` -> https://lovetypes.tw/keepsakes/#keepsake-iris
- 下一步：先發布與回填 KPI；未達門檻前不製作或加強此商品假設。

### 艾莉絲 / owned_lead

- 狀態：HOLD
- 假設：重複補給或 Contact 需求可轉成自有 Email 素材。
- 目前數據：quiz=0，supply_lead_requests=0，contact_requests=0
...
- 狀態：HOLD
- 假設：Luna 使用場景可在結果後路線承接夜間整理需求。
- 目前數據：quiz=0，luna_pack_clicks=0，luna_clicks=0
...
- 狀態：HOLD
- 假設：延伸書卷可承接願意深讀的使用者，但不應取代免費路線。
- 目前數據：quiz=0，affiliate_book_clicks=0，resources_clicks=0
