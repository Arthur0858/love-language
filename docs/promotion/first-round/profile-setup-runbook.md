...
```bash
python3 tools/promotion_profile_writeback.py update --platform youtube_shorts --status set --set-date 2026-07-02 --proof-note "<REAL_SCREENSHOT_OR_PROFILE_CLICK_NOTE> verified"
```

公開可點後：

```bash
python3 tools/promotion_profile_writeback.py update --platform youtube_shorts --status live --set-date 2026-07-02 --proof-note "<REAL_PROFILE_CLICK_NOTE> verified"
```
