...
- `refresh_ops_docs` / `ready`：`python3 tools/promotion_daily_ops_refresh.py`
  Stop: Do not publish from stale generated packets after tracker writeback.
- `verify_profile_gates` / `complete`：`python3 tools/promotion_profile_completion_gate.py --check && python3 tools/promotion_profile_publish_handoff.py --check && python3 tools/promotion_launch_readiness_gate.py --check`
  Stop: Publish only when all profile gates agree ready_to_publish is open.
- `open_first_batch_publish` / `ready`：`python3 tools/promotion_first_batch_publish_quickstart.py --check`
...
- direct set：`python3 tools/promotion_profile_writeback.py update --platform youtube_shorts --status set --set-date 2026-06-28 --proof-note "<REAL_SCREENSHOT_OR_PROFILE_CLICK_NOTE> verified"`
- direct live：`python3 tools/promotion_profile_writeback.py update --platform youtube_shorts --status live --set-date 2026-06-28 --proof-note "<REAL_PROFILE_CLICK_NOTE> verified"`
