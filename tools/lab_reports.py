...
LAB_SECTION_TITLES = {
    "quiz-scoring-test": {
        "environment": "計分測試在哪些環境執行",
        "fixture": "兩組答案序列與人工預期",
        "steps": "從清除資料到重放同分案例",
        "results": "五類分數與排序逐項對照",
        "raw": "score 物件與畫面觀察",
        "acceptance": "哪些差異會判定計分失敗",
        "retest": "題庫變更後的回歸路徑",
        "evidence": "本報告保存的計分證據",
        "failure": "結果理解曾被什麼干擾",
        "fix": "如何收斂測驗後續選項",
        "limitations": "計分正確仍不能證明什麼",
        "method": "這份計分紀錄的性質與版本",
        "next": "接著檢查其他測驗行為",
    },
    "result-consistency-test": {
        "environment": "重測使用的瀏覽器與視窗",
        "fixture": "A、B、C 三組序列如何設計",
        "steps": "如何隔離每一輪保存狀態",
        "results": "完全重複與單題變更結果",
        "raw": "三輪原始分數差異",
        "acceptance": "何時不能聲稱輸出一致",
        "retest": "資料或版面改動後怎麼重放",
        "evidence": "一致性比較保留哪些資料",
        "failure": "固定類型文案造成的誤讀",
        "fix": "如何呈現接近同分與變動",
        "limitations": "軟體穩定不等於重測信度",
        "method": "一致性報告的適用版本",
        "next": "比較其他產品測試證據",
    },
    "local-storage-privacy-test": {
        "environment": "一般、無痕與獨立內容區",
        "fixture": "保存鍵與唯一辨識字串",
        "steps": "保存、重載、清除與跨區檢查",
        "results": "四種瀏覽器狀態的資料結果",
        "raw": "Application 與 Network 觀察",
        "acceptance": "什麼情況構成本機隱私失敗",
        "retest": "保存鍵或分析服務變更後重測",
        "evidence": "清除前後如何留下證據",
        "failure": "共用裝置仍可能暴露什麼",
        "fix": "清除控制與風險提示如何補足",
        "limitations": "localStorage 不是加密保險箱",
        "method": "本機保存報告的版本邊界",
        "next": "繼續核對分享與輸出範圍",
    },
    "share-card-privacy-test": {
        "environment": "四條分享路徑的攔截環境",
        "fixture": "私人辨識字串與 Claire 結果",
        "steps": "送出前如何檢查每種 payload",
        "results": "文字、複製、圖片與網址對照",
        "raw": "分享輸出逐字搜尋結果",
        "acceptance": "哪些欄位一出現就算失敗",
        "retest": "更換守護者後如何再驗證",
        "evidence": "分享證據保存到哪個邊界",
        "failure": "角色名稱也可能是敏感資訊",
        "fix": "如何讓使用者先看見分享範圍",
        "limitations": "外部平台接收後不由本站控制",
        "method": "分享隱私紀錄的適用 build",
        "next": "查看其他資料流向測試",
    },
    "compatibility-safety-test": {
        "environment": "羅盤配對與畫面測試條件",
        "fixture": "五組配對與禁用語清單",
        "steps": "交換順序與未完成輸入怎麼測",
        "results": "對稱建議與越權語言檢查",
        "raw": "五組輸出的人工閱讀紀錄",
        "acceptance": "何種結論會越過工具能力",
        "retest": "配對文案更新後的安全掃描",
        "evidence": "交換前後與禁用語證據",
        "failure": "相容性名稱曾造成的命定誤解",
        "fix": "如何把工具收斂為溝通羅盤",
        "limitations": "兩個入口不能判斷關係去留",
        "method": "羅盤安全報告的適用範圍",
        "next": "比較其他安全與可用性測試",
    },
    "repair-plan-usability-test": {
        "environment": "360 與 390 像素手機條件",
        "fixture": "短句、120 字與回看日期輸入",
        "steps": "從空白表單走到清除完成",
        "results": "欄位、鍵盤、保存與長文表現",
        "raw": "兩種窄螢幕的操作觀察",
        "acceptance": "哪些中斷會讓工作表失敗",
        "retest": "欄位或固定導覽改版後重走",
        "evidence": "短輸入、壓力輸入與清除畫面",
        "failure": "完成區曾出現的下一步干擾",
        "fix": "如何保留免費且明確的完成出口",
        "limitations": "手機模擬沒有涵蓋所有鍵盤",
        "method": "工作表可用性紀錄的版本",
        "next": "繼續檢查操作與無障礙證據",
    },
    "keyboard-accessibility-test": {
        "environment": "鍵盤、對比與動畫偏好條件",
        "fixture": "按鍵序列與 WCAG 對比門檻",
        "steps": "不用滑鼠走過主要命令",
        "results": "焦點順序、可見性與動畫結果",
        "raw": "正向與反向巡覽觀察",
        "acceptance": "哪些障礙不能標示通過",
        "retest": "互動元件改版後如何再巡覽",
        "evidence": "焦點框、色值與文字清單",
        "failure": "自動工具無法代替哪些檢查",
        "fix": "焦點與減少動態防護如何維持",
        "limitations": "尚未涵蓋的輔助科技",
        "method": "無障礙工程紀錄不是認證",
        "next": "接著查看失敗模式測試",
    },
    "slow-network-performance-test": {
        "environment": "Fast 3G、壞圖與無腳本條件",
        "fixture": "節流數值與被阻擋資產",
        "steps": "三輪失敗模式如何分開執行",
        "results": "慢速、圖片失敗與無腳本結果",
        "raw": "正文與工具最低可用狀態",
        "acceptance": "哪些空白或假結果算失敗",
        "retest": "新增必要腳本後如何重跑",
        "evidence": "失敗請求與無腳本畫面範圍",
        "failure": "網路或資產失敗暴露的風險",
        "fix": "HTML 正文與固定尺寸如何降級",
        "limitations": "工程節流不能代表真實訪客",
        "method": "效能失敗紀錄的適用版本",
        "next": "返回實測總覽比較限制",
    },
}
...
for _report in LAB_REPORTS:
    ...
    _report["section_titles"] = LAB_SECTION_TITLES[_report["slug"]]
    _report["environment"] = [
