...
LAB_REPORTS = [
    ...
    },
    {
        ...
        "environment": ["正式繁中相容性工具", "桌面 1440×900 與手機 360×800", "五組固定守護者配對", "不輸入姓名或聯絡資料"],
        ...
        "failure": "舊版頁面名稱容易讓人把「相容性」理解成命運評分，而且其他導流按鈕出現得太早。",
        "fix": "第一屏改以溝通羅盤定位，結果只提供錯頻翻譯與十分鐘對話；主要流程不插入其他報告索取入口。",
...
LAB_TEST_DETAILS = {
    ...
    "compatibility-safety-test": {
        ...
        "fixture": "固定組合為 W×W、W×T、T×W、G×S、P×W，關係狀態依序選交往中、遠距、衝突後、同住與尚未定義。逐一搜尋相配率、百分比、命定伴侶、關係預言、關係去留指令及保證等字詞。",
        "raw_observation": "所有完整輸入都產生共同理解、可能錯頻、可說句子與今日行動，不輸出相配分數或未來預言。W×T 與 T×W 的角色描述隨位置交換，但核心差異與安全提醒一致。未選完整時按鈕不產生結果。頁面未要求姓名或聯絡資料，也沒有其他報告索取 CTA。",
        "method_detail": "羅盤只組合兩個自填接收入口與當下情境，無法評估價值觀、權力、責任分配或安全。測試重點是防止輸出越過這個資料能力：不以角色差異推論適合程度，不把同類型說成最佳配對，也不把任何結果包裝成命定預測。安全關鍵字檢查與人工閱讀都必須通過。",
        "revision": "移除與核心任務無關的推算入口，加入五組固定配對、交換順序及禁用語逐字檢查。",
...
LAB_ACCEPTANCE = {
    ...
    "compatibility-safety-test": (
        "驗收條件是所有配對只提供描述、翻譯與可拒絕的小請求，不出現排名、機率、命定結論、未提供資料的推算或關係去留指令。交換左右順序時可以改變稱呼，但不能把同一差異變成一方優越；未完成輸入不得產生看似個人化的結果。",
        "每次文案或配對資料更新後，先重跑五組固定組合，再以禁用詞清單掃描 HTML、JavaScript 與實際結果。人工閱讀要確認安全聲明沒有被折疊或放在其他導流區塊後面。若未來新增更多情境，也只能擴充溝通選項，不能把缺少的價值觀、責任或安全資料推算出來。",
    ),
    "repair-plan-usability-test": (
