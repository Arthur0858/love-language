...
PRESERVE_FIELDS = {"status", "scheduled_date", "published_date", "post_url", "notes"}
...
def build_row(task: dict, platform: str, existing: dict[str, str] | None = None) -> dict[str, str]:
...
def build_rows(tasks: list[dict], existing: dict[tuple[str, str], dict[str, str]]) -> list[dict[str, str]]:
    rows: list[dict[str, str]] = []
    sorted_tasks = sorted(tasks, key=lambda task: (int(task.get("week", 0) or 0), int(task.get("slot", 0) or 0)))
    for task in sorted_tasks:
        for platform in PLATFORMS:
            rows.append(build_row(task, platform, existing.get((platform, str(task.get("taskId", ""))))))
    return rows
...
def validate_rows(rows: list[dict[str, str]]) -> list[str]:
    ...
    for row in rows:
        ...
        seen.add(key)
        for field in ("week", "slot", "platform", "status", "task_id", "script_id", "guardian_id", "title", "caption", "tracked_url", "utm_content"):
...
def main() -> int:
    ...
    rows = build_rows(load_tasks(Path(args.kit)), read_existing(Path(args.output)))
